...
class PLYLoader extends Loader {
	...
	parse( data ) {
		...
		function postProcess( buffer ) {
			...
			if ( buffer.class.length > 0) {

				geometry.setAttribute('class',new Float32BufferAttribute( buffer.class, 1 ))
				// if (buffer.class === 0){
				// 	geometry.setAttribute('segmention',new Float32BufferAttribute( buffer.class, 1 ))
				// }
			}
		...
		function handleElement( buffer, elementName, element ) {
			...
			const attrC = findAttrName( [ 'scalar_Scalar_field', 'label', 'scalar_Constant'] );
			...
			if ( elementName === 'vertex' ) {
				...
				if ( attrC !== null) {
					buffer.class.push(element[ attrC ]);
				}
