...
class computeFaceNormal {
	constructor(geometry, face) {
		...
		var i = 0;
		var vA = vertexa.fromBufferAttribute(positionAttribute, face.a );
		var vB = vertexb.fromBufferAttribute(positionAttribute, face.b );
		var vC = vertexc.fromBufferAttribute(positionAttribute, face.c );
		cb.subVectors(vC, vB);
		ab.subVectors(vA, vB);
		cb.cross(ab);
		cb.normalize();
